...
void PinochleGame::suit_independent_evaluation(const CardSet<Suit, PinochleRank> &pino_player_hand, vector<PinochleMelds> &pino_melds_vector)
{
    ...
    vector<Card<Suit, PinochleRank>>::iterator cardSet_itr_begin = temp_card_set.begin();
    vector<Card<Suit, PinochleRank>>::iterator cardSet_itr_end = temp_card_set.end();
    ...
    while (cardSet_itr_begin != cardSet_itr_end){
        const auto &card = *cardSet_itr_begin;
